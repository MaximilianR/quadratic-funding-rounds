...
library(magrittr)
library(tidyverse)
# in the data frame people have sometimes multiple votes per proposal because of how I constructed the simulation, so I have to fix this first by combining them before doing the QF
...
**With those funding percentages per proposal, we can now see if the proposal will get funding. But it depends on what the proposal's target is, and how much money is in the funding pool. If there is little money in the pool, the grant might not get passed the 'min' value, and in that case it won't get funding.**
...
**If there isn't much money in the pool, and people vote randomly, likely only the proposals asking for little money will get any.**
...
**If there is matched money in the pool, and people vote randomly, the proposals asking for a lot of money still might not get pass their min ask.**
...
**If there isn't much money in the pool, the popular proposals are much more likely to pass.**
...
**If there is matched money in the pool, the popular proposals are still much more likely to pass and pushes out the others.**
...
**If there is a lot of money in the pool, the popular proposals are still much more likely to pass and pushes out the others to fail.**

So bad proposals are still likely not to get funded.
...
- It matters how people vote. Even with little money in the pool, the popular proposals have a good chance. And with a lot of money in the pool, the unpopular proposals will likely not be funded